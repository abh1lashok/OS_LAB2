...
void main (int argc, char *argv[])
{
  ...
  lock_t prod_cons_lock[NO_OF_PROCESSES];
  char prod_cons_lock_str[NO_OF_PROCESSES][10];
  ...
  for (i =0; i < numprocs;i++){
    if ((prod_cons_lock[i] = lock_create()) == SYNC_FAIL) {
    Printf("Lock Not Implemented "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  ditoa(prod_cons_lock[i], prod_cons_lock_str[i]);
  }
  ...
  for(i=0; i<1; i++) {
    process_create(PRODUCER_TO_RUN, h_mem_str, s_procs_completed_str,lock_str,prod_cons_lock_str[i], NULL);
    Printf("Process %d created\n", i);
  }

  for(i=0; i<numprocs; i++) {
    process_create(CONSUMER_TO_RUN, h_mem_str, s_procs_completed_str,lock_str,prod_cons_lock_str[i], NULL);
    Printf("Process %d created\n", i);
  }
