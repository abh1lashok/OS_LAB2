...
static Cond cond_vars[MAX_CONDS];
...
int CondInit(Cond* cond_var){



  // cond_var->lock = lock;
if (!cond_var) return SYNC_FAIL;
  if (AQueueInit (&cond_var->waiting) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not initialize conditional Variable waiting queue in ConVarInit!\n");
    exitsim();
  }
  return SYNC_SUCCESS;
}
...
int CondHandleWait(cond_t c) {
  ...
  else if(!CondWait(&cond_vars[c])) return SYNC_FAIL;
  return SYNC_SUCCESS;
}


int CondWait(Cond* cond_var){
  Link* l;
  int intrval;
  // Release Lock
...
int CondHandleSignal(cond_t c) {
  ...
  if(c >= MAX_CONDS) return SYNC_FAIL;
  else if(!CondSignal(&cond_vars[c])) return SYNC_FAIL;
  return SYNC_SUCCESS;
